...
def test_offline_requires_dataset_name():
    """Test that OFFLINE mode requires dataset_name."""
    print("Executing test: OFFLINE requires dataset_name")

    with pytest.raises(ValueError, match="dataset_name is required for OFFLINE mode"):
        SpeculativeDecodingTrainer(
            verifier_model="Qwen/Qwen3-8B",
...
def test_offline_valid():
    """Test OFFLINE mode with user-provided vllm_endpoint."""
    print("Executing test: OFFLINE valid with endpoint")
...
def test_offline_valid_without_endpoint():
    """Test OFFLINE mode with managed vLLM (no endpoint provided)."""
    print("Executing test: OFFLINE valid without endpoint")
...
def test_offline_renders_both_functions():
    """Test that OFFLINE generates script with both data and training calls."""
    print("Executing test: OFFLINE renders both functions")
...
def test_offline_renders_vllm_endpoint():
    """Test that OFFLINE script passes vllm_endpoint to data extraction."""
    print("Executing test: OFFLINE renders vllm_endpoint")
...
def test_offline_auto_derives_hidden_states_path():
    """Test that OFFLINE auto-derives hidden_states_path from output_dir."""
    print("Executing test: OFFLINE auto-derives hidden_states_path")
    ...
    assert "hidden_states_path='/mnt/kubeflow-checkpoints/output'" in script

    print("test execution complete")


def test_dataset_name_pvc_uri_resolves():
    """Test that dataset_name with pvc:// URI is resolved in the generated script."""
    print("Executing test: dataset_name PVC URI resolves")

    trainer = SpeculativeDecodingTrainer(
        verifier_model="Qwen/Qwen3-8B",
        mode=SpeculatorMode.OFFLINE,
        dataset_name="pvc://test-pvc/data/prompts.jsonl",
        output_dir="pvc://test-pvc/output",
        vllm_endpoint="http://vllm-svc:8000/v1",
    )
    ...
    assert "dataset_name='/mnt/kubeflow-checkpoints/data/prompts.jsonl'" in script

    print("test execution complete")


def test_dataset_name_plain_string_passthrough():
    """Test that dataset_name without pvc:// is passed through as-is."""
    print("Executing test: dataset_name plain string passthrough")
...
def test_dataset_name_local_path_passthrough():
    """Test that dataset_name with local path is passed through as-is."""
    print("Executing test: dataset_name local path passthrough")

    trainer = SpeculativeDecodingTrainer(
        verifier_model="Qwen/Qwen3-8B",
        mode=SpeculatorMode.OFFLINE,
        dataset_name="/data/prompts.jsonl",
        output_dir="pvc://test-pvc/output",
        vllm_endpoint="http://vllm-svc:8000/v1",
    )
    ...
    assert "dataset_name='/data/prompts.jsonl'" in script

    print("test execution complete")


def test_dataset_name_unsupported_scheme():
    """Test that dataset_name with unsupported URI scheme raises error."""
    print("Executing test: dataset_name unsupported scheme")

    with pytest.raises(NotImplementedError, match="dataset_name scheme"):
        SpeculativeDecodingTrainer(
            verifier_model="Qwen/Qwen3-8B",
            mode=SpeculatorMode.OFFLINE,
            dataset_name="s3://bucket/prompts.jsonl",
            output_dir="pvc://test-pvc/output",
            vllm_endpoint="http://vllm-svc:8000/v1",
        )
...
def test_offline_uses_torch_command():
    """Test that OFFLINE mode uses TORCH_COMMAND (torchrun) for multi-GPU support."""
    print("Executing test: OFFLINE uses TORCH_COMMAND")
...
def test_offline_progression_data_phase(tmp_path):
    ...
    handler = handler_class.__new__(handler_class)
    result = handler._data_progress(scale=50, offset=0)

    assert result["currentStep"] == 5
    assert result["totalSteps"] == 10
    assert result["progressPercentage"] == 25
...
def test_offline_script_calls_both_functions():
    """Test that OFFLINE script calls both data extraction and training functions."""
    print("Executing test: OFFLINE script calls both functions")
    ...
    assert "_speculator_data_only(" in script
    assert "_speculator_train_only(" in script
    assert "if _rank == 0:" not in script
